...
import { X, Loader2, Chrome, LogIn } from 'lucide-react';
...
interface AuthModalProps {
  isOpen: boolean;
  onClose: () => void;
  onMockLogin: (user: any) => void; // Mantido para compatibilidade de interface
}
...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onMockLogin }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // O App.tsx via onAuthStateChange cuidará do resto após o retorno.
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm bg-white rounded-[40px] shadow-2xl overflow-hidden border border-white/20 animate-fade-in p-10">
        ...
        <div className="text-center mb-10">
            ...
            <p className="text-slate-400 text-sm">Use sua conta Google para sincronizar suas análises no Supabase.</p>
        </div>
        ...
        <div className="space-y-4">
            <button 
                ...
                disabled={loading}
                className="w-full flex items-center justify-center gap-4 bg-white border border-slate-200 p-4 rounded-2xl hover:bg-slate-50 transition-all font-bold text-slate-700 disabled:opacity-50 shadow-sm"
            >
                 {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Chrome className="w-6 h-6 text-[#4285F4]" />}
            ...
            <p className="text-[10px] text-slate-400 text-center px-4 leading-relaxed">
              Logando como: <b>evaldo0510@gmail.com</b> ou <b>aljariristartups@gmail.com</b> você terá acesso administrativo.
            </p>
        </div>
      </div>
    </div>
